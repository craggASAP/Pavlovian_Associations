...
close all;clear;clc;
cf = [pwd,'\'];
common_plot_functions.component_time_histogram(cf,["cue1early","cue1late"],'sup_fig1A');
...
close all;clear;clc;
cf = [pwd,'\'];
common_plot_functions.component_time_histogram(cf,["cue2early","cue2late"],'sup_fig1B');
...
close all;clear;clc;
cf = [pwd,'\'];
common_plot_functions.component_presence_circle(cf,"pav2cue task",["cue1early","cue1late"],'sup_fig1C');
...
close all;clear;clc;
cf = [pwd,'\'];
common_plot_functions.component_presence_circle(cf,"pav2cue task",["cue2early","cue2late"],'sup_fig1D');
...
plot_colors = parula(3); pci = 1;
fig = figure(Position=[100,100,640,360]);
tiled = tiledlayout(fig,1,1,TileSpacing="tight");
axes=gobjects(1,1);
for i=1:1
...
for n_mouse_name_i = 1:n_mouse_name
    ...
    for n_phase_name_i = 1:n_phase_name
        ...
        axis(axs([1,4]),"vis3d")
        axis(axs([2,5]),"vis3d")
        saveas(circle_fig,[cf,'sup_fig1F_',char(phase_name),'.png']);
        delete(circle_fig)
    end
end
